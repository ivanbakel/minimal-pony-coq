...
Definition context : Type := (varMap Syntax.aliasedType).
    ...
    (fun var varType sendableMap =>
      ...
      | Syntax.aType _ b =>
          if Syntax.isSendable b
            then VarMap.add var varType sendableMap
            else sendableMap
      end)
    (VarMap.empty Syntax.aliasedType)
    gamma.
...
Inductive typing { P : Program.program } : forall ( X : Type), context -> X -> Syntax.ponyType -> context -> Prop :=
  ...
  : ~ VarMap.In x gamma
      -> gamma |-{ Syntax.expression } Syntax.varDecl x : Syntax.asPonyType aT ==> (VarMap.add x aT gamma)
